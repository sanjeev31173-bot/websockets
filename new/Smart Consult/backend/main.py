...
async def consultation_ws(websocket: WebSocket, session_id: str) -> None:
    ...
    - Client sends:
        ...
        * A final text frame "END" to close the stream.
    - Server sends:
        * Text frames with incremental transcript updates as they are produced.
        * A final JSON text frame {"type": "end"} once transcription is finished.
    """
    await websocket.accept()
    # IMPORTANT: one WebSocket connection == one session.
    # This prevents multiple clients attaching to the same session_id and corrupting audio flow.
        ...
        while True:
            ...
            if "bytes" in msg and msg["bytes"] is not None:
                ...
                if text == "END":
                    await session.mark_closed()
                    break
                # Optional: client can send {"type":"start","sample_rate_hz":48000}
...
async def _send_transcript_updates(websocket: WebSocket, session: AudioSession) -> None:
    ...
    # Persist transcript to disk on the server machine (your laptop).
    ...
    text = " ".join(t for t in session.full_transcript if not t.startswith("[transcription_warning]")).strip()
